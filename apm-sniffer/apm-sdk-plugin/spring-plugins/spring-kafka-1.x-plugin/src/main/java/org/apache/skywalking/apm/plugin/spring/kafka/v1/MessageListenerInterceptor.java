...
public class MessageListenerInterceptor implements InstanceMethodsAroundInterceptor {
    ...
    public static final String CONSUMER_OPERATE_NAME_SUFFIX = "/Listener";
    ...
    public void beforeMethod(EnhancedInstance objInst, Method method, Object[] allArguments,
        ...
        AbstractSpan activeSpan =
                ContextManager.createEntrySpan(OPERATE_NAME_PREFIX + record.topic() +
                        CONSUMER_OPERATE_NAME_SUFFIX, contextCarrier).start(System.currentTimeMillis());
        activeSpan.setComponent(ComponentsDefine.KAFKA_CONSUMER);
        SpanLayer.asMQ(activeSpan);
        String peerHost = contextCarrier.getPeerHost();
        if (!StringUtil.isEmpty(peerHost) && peerHost.startsWith("#")) {
            peerHost = peerHost.substring(1);
        }
        Tags.MQ_BROKER.set(activeSpan, peerHost);
