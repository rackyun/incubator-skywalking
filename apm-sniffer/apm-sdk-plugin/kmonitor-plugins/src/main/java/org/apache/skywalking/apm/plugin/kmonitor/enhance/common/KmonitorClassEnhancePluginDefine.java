...
package org.apache.skywalking.apm.plugin.kmonitor.enhance.common;
...
public abstract class KmonitorClassEnhancePluginDefine extends ClassInstanceMethodsEnhancePluginDefine {
    ...
    private DynamicType.Builder<?> enhanceInstance(TypeDescription typeDescription,
        ...
        if (existedMethodsInterceptPoints) {
            ElementMatcher.Junction<MethodDescription> crossJunction = any();
            Implementation.Composable crossComposable = null;
            for (InstanceMethodsInterceptPoint instanceMethodsInterceptPoint : instanceMethodsInterceptPoints) {
                ...
                crossJunction = crossJunction.and(junction);
                Implementation.Composable composable;
                if (instanceMethodsInterceptPoint.isOverrideArgs()) {
                    composable = MethodDelegation.withDefaultConfiguration()
                            .withBinders(Morph.Binder.install(OverrideCallable.class))
                            .to(new InstMethodsInterWithOverrideArgs(interceptor, classLoader));
                    newClassBuilder = newClassBuilder.method(junction).intercept(composable);
                } else {
                    composable = MethodDelegation.withDefaultConfiguration()
                            .to(new InstMethodsInter(interceptor, classLoader));
                    newClassBuilder = newClassBuilder.method(junction).intercept(composable);
                }
                if (crossComposable == null) {
                    crossComposable = composable;
                } else {
                    crossComposable = crossComposable.andThen(MethodDelegation.withDefaultConfiguration().to(composable));
                }
                logger.debug("enhance class {} junction {} interceptor {}",
                        enhanceOriginClassName, junction, instanceMethodsInterceptPoint.getMethodsInterceptor());
            }
            if (crossComposable != null) {
                newClassBuilder.method(crossJunction).intercept(crossComposable);
            }
        }
