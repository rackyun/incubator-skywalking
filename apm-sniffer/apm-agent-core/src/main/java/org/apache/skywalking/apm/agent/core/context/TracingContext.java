...
public class TracingContext implements AbstractTracerContext {
    ...
    private static final String IGNORE_PARENT_OPERATION_NAME = "Hystrix";
    ...
    public void inject(ContextCarrier carrier) {
        ...
        if (logger.isDebugEnable()) {
            for (Iterator<AbstractSpan> it = iterator(); it.hasNext(); ) {
                AbstractSpan abstractSpan = it.next();
                logger.debug("inject list span, operationName={}, operationId={},  isEntry={}",
                        abstractSpan.getOperationName(), abstractSpan.getOperationId(), abstractSpan.isEntry());
                for (TraceSegmentRef ref : abstractSpan.getRefs()) {
                    logger.debug("abstract span traceSegmentRef endpointId={}, endpointName={}",
                            ref.getEntryEndpointId(), ref.getEntryEndpointName());
                }
            }
            if (refs != null) {
                for (TraceSegmentRef ref : refs) {
                    logger.debug("traceSegmentRef endpointName={}, serviceInstanceId={}",
                            ref.getEntryEndpointName(), ref.getEntryServiceInstanceId());
                }
            }
        }
        ...
        if (logger.isDebugEnable()) {
            logger.debug("carrier entryEndpointName={}, parentOperationName={}",
                    carrier.getEntryEndpointName(), carrier.getParentEndpointName());
        }
     ...
     *
     * @param snapshot from {@link #capture()} in the parent thread. Ref to
     * {@link AbstractTracerContext#continued(ContextSnapshot)}
     */
    @Override
    public void continued(ContextSnapshot snapshot) {
        if (logger.isDebugEnable()) {
            logger.debug("tracing context continued entryOperationName={}, parentOperationName={}, spanId={}",
                                    ...
                                    return DictionaryManager.findEndpointSection()
                                            ...
                                            .doInCondition(
                                                    new PossibleFound.FoundAndObtain() {
                                                        @Override
                                                        public Object doProcess(int operationId) {
                                                            return new ExitSpan(spanIdGenerator++, parentSpanId,
                                                                    operationId, peerId);
                                                        }
                                                    }, new PossibleFound.NotFoundAndObtain() {
                                                        @Override
                                    ...
                                    return DictionaryManager.findEndpointSection()
                                            ...
                                            .doInCondition(
                                                    new PossibleFound.FoundAndObtain() {
                                                        ...
                                                        public Object doProcess(int operationId) {
                                                            return new ExitSpan(spanIdGenerator++, parentSpanId,
                                                                    operationId, remotePeer);
                                                        }
                                                    }, new PossibleFound.NotFoundAndObtain() {
                                                        @Override
                                                        public Object doProcess() {
                                                            return new ExitSpan(spanIdGenerator++, parentSpanId,
                                                                    operationName, remotePeer);
                                                        }
                                                    });
                                }
                            });
    ...
    private Iterator<AbstractSpan> iterator() {
